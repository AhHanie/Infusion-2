...
namespace Infusion
{
    ...
    {
        private static FieldInfo compsField;

        private static FieldInfo compsByTypeField;
        ...
        public static void NotifyApparelAddedPostfix(Apparel apparel)
        {
            ...
            {
                List<ThingComp> list = (List<ThingComp>)compsField.GetValue(apparel);
                if (!list.Any((ThingComp x) => x is CompShield))
                {
                    ...
                    list.Add(thingComp);
                    Dictionary<Type, ThingComp[]> dictionary = (Dictionary<Type, ThingComp[]>)compsByTypeField.GetValue(apparel);
        ...
        public static void InitRefelection()
        {
            compsField = AccessTools.Field(typeof(ThingWithComps), "comps");
            compsByTypeField = AccessTools.Field(typeof(ThingWithComps), "compsByType");
        }
