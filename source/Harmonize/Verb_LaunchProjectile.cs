...
namespace Infusion.Harmonize
{
    ...
    public static class TryCastShot_SmartInfusion
    {
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            Log.Message($"[Infusion] Starting transpiler. Total instructions: {codes.Count}");
            ...
            for (int i = 0; i < codes.Count - 3; i++)
            {
                ...
                {
                    insertIndex = i;
                    Log.Message($"[Infusion] Found ForcedMissRadius check pattern at index {i}");
                    break;
                }
            }
            ...
            if (continueInstruction.labels.Count > 1)
            {
                ...
                if (labelsToMove.Count > 0)
                {
                    Log.Message($"[Infusion] Moved {labelsToMove.Count} labels to our first instruction");
                }
            }
